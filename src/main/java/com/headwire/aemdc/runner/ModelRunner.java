...
public class ModelRunner extends BasisRunner {
   ...
   *          - resource object
   */
  public ModelRunner(final Resource resource) {
    this.resource = resource;

    // Get Config Properties from config file
    configProps = ConfigUtil.getConfigProperties();

    LOG.debug("Model runner starting...");
  ...
  public boolean checkConfiguration() {
    ...
    if (StringUtils.isBlank(targetPath)) {
      LOG.error("The target folder {} is blank in the config file.", Constants.CONFIGPROP_TARGET_MODELS_FOLDER);
      return false;
    }
